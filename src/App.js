...
export default function ChartAnalyzer() {
  ...
  const analyze = async () => {
    ...
    const prompt = `あなたはSMC（Smart Money Concepts）とCRT（Candle Range Theory）の専門家です。
このチャート画像（時間足: ${timeframe}${pair ? `、通貨ペア: ${pair}` : ""}）を分析してください。

以下の項目について、必ずJSON形式のみで回答してください（前置き・説明・マークダウン不要）:

{
  "direction": "BUY" or "SELL" or "NEUTRAL",
  "confidence": 0〜100の数値,
  "patterns": {
    "chochBos": { "found": true or false, "detail": "説明" },
    "ob": { "found": true or false, "detail": "説明" },
    "liquiditySweep": { "found": true or false, "detail": "説明" },
    "eqhEql": { "found": true or false, "detail": "説明" },
    "qm": { "found": true or false, "detail": "説明" },
    "sbrRbs": { "found": true or false, "detail": "説明" },
    "mss": { "found": true or false, "detail": "説明" },
    "turtleSoup": { "found": true or false, "detail": "説明" }
  },
  "entryPoint": {
    ...
    "stopLoss": "損切りの目安",
    "takeProfit": "利確の目安",
    "reasoning": "エントリー根拠の説明"
  },
  "summary": "市場全体の状況を2〜3文で説明"
}`;
    ...
    <div style={{ minHeight: "100vh", background: "#07090f", color: "#cbd5e1", fontFamily: "'Courier New', monospace", padding: "0 0 60px" }}>
      ...
      <div style={{ background: "linear-gradient(180deg, #0d1117 0%, #07090f 100%)", borderBottom: "1px solid #1e2a3a", padding: "20px" }}>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          ...
          <div style={{ fontSize: 11, color: "#475569", marginTop: 4 }}>チャートをアップロードしてAI分析を実行</div>
        </div>
      </div>
